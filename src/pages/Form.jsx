...
function Form() {
  ...
  const handleCheckbox = (merk) => {
    if (rokok.includes(merk)) {
      setRokok(rokok.filter((item) => item !== merk));
    } else {
      setRokok([...rokok, merk]);
    }
  };
      ...
      <div className="w-full max-w-2xl mx-4">
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <div className="bg-white p-6 rounded-lg shadow-sm border border-slate-200">
            <label className="block text-base mb-4">
            ...
            <div className="space-y-3">
              {["Sampurna", "Kretek", "Filter", "Grandel"].map((merk) => (
                <label key={merk} className="flex items-center cursor-pointer">
                  <input
                    type="checkbox"
                    checked={rokok.includes(merk)}
                    onChange={() => handleCheckbox(merk)}
          ...
          <div className="flex justify-between items-center py-4">
            <button
              type="submit"
              className="bg-purple-700 text-white px-6 py-2 rounded shadow hover:bg-purple-800 transition font-medium"
            >
              Kirim
            </button>
            <button
              type="reset"
              className="text-purple-700 font-medium text-sm hover:bg-purple-50 px-3 py-2 rounded"
              onClick={() => {
                setNama("");
                setUmur("");
                setJk("");
